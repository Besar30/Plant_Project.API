...
public class CartServices(UserManager<ApplicationUser> userManager,
    ApplicationDbContext context, IHttpContextAccessor httpContextAccessor) : ICartServices
{
    ...
    public async Task<Result<List<CartResponse>>> GetAllAsync(string userId, CancellationToken cancellationToken = default)
    {
        var cartItems = await _context.Carts
            ...
            .Select(c => new CartResponse(
                ...
                c.Price,
                c.Quantity * c.Price,
               $"{_httpContextAccessor.HttpContext!.Request.Scheme}://{_httpContextAccessor.HttpContext.Request.Host}{c.ImagePath}"  
